...
import { forgotPassOtpSend } from "../components/admin/user/user";
...
const VerifyPhone = () => {
    ...
    const handleSubmit = async ({ phone }) => {
        phone = "+880" + phone
        const { error, msg } = await forgotPassOtpSend(phone);
        if (error) {
            await Swal.fire('Error', msg, 'error');
        } else {
            router.push({ pathname: '/otp', query: { user: phone.substr(-11) } });
        }
    };
